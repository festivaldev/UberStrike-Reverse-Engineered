...
public class DepthOfField34 : PostEffectsBase {
	...
	protected virtual void CreateMaterials () {		
	...
	protected virtual int GetDividerBasedOnQuality () {
		int divider = 1;
	...
	protected virtual int GetLowResolutionDividerBasedOnQuality (int baseDivider) {
	...
	protected virtual void OnRenderImage (RenderTexture source, RenderTexture destination) {	
		...
		Blur (mediumRezWorkTexture, mediumRezWorkTexture, (int)DofBlurriness.Low, 4, maxBlurSpread);			

		if (bokeh && (bokehDestination & BokehDestination.Background) == BokehDestination.Background) { 		
			...
			// bigger BLUR
			Blur (lowRezWorkTexture, lowRezWorkTexture, (int)bluriness, 0, maxBlurSpread * bokehBlurAmplifier);				
		} 
		else  {
			// bigger BLUR
			Downsample (mediumRezWorkTexture, lowRezWorkTexture);			
			Blur (lowRezWorkTexture, lowRezWorkTexture, (int)bluriness, 0, maxBlurSpread);	
		}	
		...
		if (blurForeground) {			
			...
			Downsample (source, mediumRezWorkTexture);	
       		...
       		// 1.) reduce jitter, noise, aliasing
       		// 2.) produce the little-blur buffer used in composition later   
       		BlurFg (mediumRezWorkTexture, mediumRezWorkTexture, (int)DofBlurriness.Low, 2, maxBlurSpread);	
       		
	 		if (bokeh && (bokehDestination & BokehDestination.Foreground) == BokehDestination.Foreground) { 
				...
				// big BLUR		
				BlurFg (lowRezWorkTexture, lowRezWorkTexture, (int)bluriness, 1, maxBlurSpread * bokehBlurAmplifier);				
			} 
			else  {      		
       			// big BLUR		
				BlurFg (mediumRezWorkTexture, lowRezWorkTexture, (int)bluriness, 1, maxBlurSpread);	
			}
	...
	protected virtual void Blur (RenderTexture from, RenderTexture to, int iterations, int blurPass, float spread) {
	...
	protected virtual void BlurFg (RenderTexture from, RenderTexture to, int iterations, int blurPass, float spread) {
	...
	protected virtual void BlurHex (RenderTexture from, RenderTexture to, int blurPass, float spread, RenderTexture tmp) {		
		dofBlurMaterial.SetVector ("offsets", new Vector4 (0.0f, spread * oneOverBaseSize, 0.0f, 0.0f));
		Graphics.Blit (from, tmp, dofBlurMaterial, blurPass);
		dofBlurMaterial.SetVector ("offsets", new Vector4 (spread / widthOverHeight * oneOverBaseSize,  0.0f, 0.0f, 0.0f));		
		Graphics.Blit (tmp, to, dofBlurMaterial, blurPass);	 
	...
	protected virtual void Downsample (RenderTexture from, RenderTexture to) {
	...
	protected virtual void AddBokeh (RenderTexture bokehInfo, RenderTexture tempTex, RenderTexture finalTarget) {
		if (bokehMaterial) {
	...
	protected virtual void AllocateTextures (bool blurForeground, RenderTexture source, int divider, int lowTexDivider) {
